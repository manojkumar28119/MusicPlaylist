...
class App extends Component {
  ...
  onClickDltBtn = id => {
    this.setState(prevState => ({
      trackList: prevState.trackList.filter(each => each.id !== id),
    }))
  }
  ...
  onChangeSearchInput = event => {
    this.setState({
      ...
      trackList: initialTracksList.filter(each =>
  ...
  render() {
    ...
    return (
      <div className="playlist-card">
        ...
        <div className="songs-playlist-card">
          <div className="heading-nd-input-card">
            ...
            <ul className="track-list">
              {trackList.map(each => (
                <PlaylistItem
                  trackItem={each}
                  key={each.id}
                  onClickDltBtn={this.onClickDltBtn}
                />
              ))}
            </ul>
          ) : (
            <div className="no-songs-card">
              <h1 className="no-songs">No songs Found</h1>
            </div>
          )}
        </div>
      </div>
